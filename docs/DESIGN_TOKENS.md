# Design Tokens — Voice Studio

Source: [`DESIGN.md`](../DESIGN.md) (ElevenLabs-inspired system) + Demo brand accent from example.com.
...
:root {
  ...
  /* Color — Demo accent (single accent per DESIGN.md spec) */
  --color-accent:         #E5001A; /* TODO: confirm exact hex from example.com brand */
...
### Warm-stone CTA (featured — uses Demo accent on hover only)
```
...
## 4. Demo accent usage rules

The design spec says "achromatic with warm undertones". The Demo accent is the single exception. Use it ONLY for:
...
- [ ] Confirm `--color-accent` exact hex from Demo brand guide (currently placeholder `#E5001A`).
