...
- [x] **P0-04** Prisma schema v1 (User, Invite, VoiceProfile, VoiceSample, ProviderConfig, Generation, GenerationSpeaker, AuditLog, Setting) + initial migration + seed script (super admin + default settings). · verified 2026-04-19 · direct main commit https://github.com/nhocconan/ai-voice-talk-generation/commit/ecbde8c
  - AC: `pnpm db:migrate && pnpm db:seed` creates SUPER_ADMIN `admin@demo.demo / Demo1234` with `forcePasswordChange=true`. Default provider seeded as XTTS_V2.
...
- [ ] **P0-07** CI pipeline (lint-types, web-unit, worker-unit, coverage gate).
  ...
  - DoD: Universal, E.
- [ ] **P0-08** Documentation audit: confirm `DESIGN_TOKENS.md` accent hex with Demo brand; procure fonts.
...
- [ ] **P1-01** Auth.js v5 credentials provider + invite-only signup flow (server + email with Resend).
  - AC: Super admin invites `test@example.com`, they receive email, click, set password, land on `/app`. Unauthenticated access to `/app/*` redirects to `/login`. Rate limit verified (5 req / 15 min / IP on `/api/auth/*`).
...
- [ ] **P1-03** Forced password change on first login.
  - AC: Seeded super admin cannot use the app until they change from `Demo1234`.
  - DoD: Universal, B.
...
- [ ] **P4-05** REST API + API keys for other Demo internal tools to call the generator.
...
Example:
```
- [x] **P0-04** Prisma schema v1 … · merged 2026-04-24 · https://github.com/demo/voice/pull/12
```
