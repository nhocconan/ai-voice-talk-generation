"""Worker entrypoint — FastAPI health server + BullMQ queue consumers."""
...
RENDER_DURATION = Histogram("voice_render_duration_seconds", "Render duration", ["provider", "kind"])
...
async def _db_update_sample(
    *, profile_id: str, version: int, output_key: str, duration_ms: int,
    score: int, detail: dict, notes: str | None,
) -> None:
            ...
            cur.execute(
                """INSERT INTO voice_samples (id, "profileId", version, "storageKey", "durationMs",
                   ...
                   SET "storageKey"=EXCLUDED."storageKey", "durationMs"=EXCLUDED."durationMs",
                       "qualityScore"=EXCLUDED."qualityScore", "qualityDetail"=EXCLUDED."qualityDetail"
                """,
                (profile_id, version, output_key, duration_ms, score, psycopg2.extras.Json(detail), notes),
            )
            cur.execute(
...
async def _db_asr_result(*, generation_id: str, segments: list) -> None:
    ...
    try:
        with conn.cursor() as cur:
            segs_json = json.dumps([{"startMs": s.start_ms, "endMs": s.end_ms, "speaker": s.speaker, "text": s.text} for s in segments])
            cur.execute(
                'UPDATE generations SET status=\'DONE\', "inputScript"=%s, "finishedAt"=now() WHERE id=%s',
...
async def _db_render_result(*, generation_id: str, output_mp3_key: str | None, output_wav_key: str | None, duration_ms: int) -> None:
    import psycopg2
    import os
    conn = psycopg2.connect(os.environ["DATABASE_URL"])
    try:
        with conn.cursor() as cur:
            cur.execute(
                'UPDATE generations SET status=\'DONE\', "outputMp3Key"=%s, "outputWavKey"=%s, "durationMs"=%s, "finishedAt"=now() WHERE id=%s',
...
async def _db_render_failed(*, generation_id: str, error: str) -> None:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                'UPDATE generations SET status=\'FAILED\', "errorMessage"=%s, "finishedAt"=now() WHERE id=%s',
                (error, generation_id),
            )
        conn.commit()
    finally:
...
async def _get_provider_for_generation(provider_id: str):
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute('SELECT name, "apiKeyEnc", config FROM provider_configs WHERE id=%s', (provider_id,))
            row = cur.fetchone()
    finally:
        conn.close()
    ...
    return get_provider(row["name"], api_key_enc=row["apiKeyEnc"], config=json.loads(row["config"]) if row["config"] else {})
...
async def handle_ingest(job_name: str, data: dict[str, Any]) -> None:
...
async def handle_asr(job_name: str, data: dict[str, Any]) -> None:
    await run_asr(
        generation_id=data["generationId"],
        source_key=data["sourceKey"],
        expected_speakers=data.get("expectedSpeakers", 2),
        result_fn=_db_asr_result,
    )


async def handle_render(job_name: str, data: dict[str, Any]) -> None:
    generation_id = data["generationId"]
    ...
    try:
        ...
        conn = psycopg2.connect(os.environ["DATABASE_URL"])
        with conn.cursor() as cur:
            cur.execute('UPDATE generations SET status=\'RUNNING\', "startedAt"=now() WHERE id=%s', (generation_id,))
        conn.commit()
        conn.close()

        speakers = await _get_speaker_sample_keys(data.get("speakers", []))
        ...
        await run_render(
            ...
            provider=provider,
            speakers=speakers,
            output=data.get("output", {"mp3": True, "wav": True, "chapters": False}),
            pacing_lock=data.get("pacingLock", False),
...
async def lifespan(app: FastAPI):
    logger.info("Worker starting", device=settings.torch_device, concurrency=settings.worker_concurrency)
    ...
    consumers[0].register("ingest.enroll", handle_ingest)
    consumers[1].register("asr.diarize", handle_asr)
    consumers[2].register("render.generation", handle_render)
...
app = FastAPI(title="Voice Worker", lifespan=lifespan)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("worker.main:app", host="0.0.0.0", port=8001, reload=False)
