...
const ADMIN_EMAIL = "admin@demo.demo"
...
async function ensureAdminLoggedIn(page: import("@playwright/test").Page) {
  ...
  for (const pw of ["Demo1234", "Demo1234!Admin"]) {
  ...
  if (page.url().includes("change-password")) {
    await page.getByLabel(/new password/i).fill("Demo1234!Admin")
    await page.getByLabel(/confirm/i).fill("Demo1234!Admin")
...
test.describe("P1-23 Happy path", () => {
  test("1. Admin can access user management", async ({ page }) => {
    ...
    await expect(page.getByText(/admin@demo/i)).toBeVisible({ timeout: 10_000 })
  })
